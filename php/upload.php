...
function createFolderName($string) {
  ...
  $string = substr($string,0,4) + date(si);
  error_log($string);
  return $string;
}
...
$folder = 'uploads/'.$foldname.date(is);
if(!is_dir($folder)){
mkdir($folder);
chmod($folder,0777);
}
...
for ($i = 0; $i < $n; $i++){
  // Check file size
  if ($_FILES["select"]["size"] > 500000) {
      echo "Files cannot exceed 10M.";
  // if everything is ok, try to upload file
  } else {
      // set path and name of new file :
      $target_file = $target_dir . basename($folder);
      ...
      if (move_uploaded_file($_FILES["select"]["tmp_name"][$i], $target_file)) {
          echo "The file ". basename( $_FILES["select"]["name"][$i]). " has been uploaded.";
          $success++;
      }
  }
}
